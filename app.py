        ...
        if not client_df.empty:
            ...
            if matching_rate_cols:
                for idx, row in rates_sheet.iterrows():
                    ...
                    # 🚨 REFRACTORED CLEAN FIX: Breaks calculation lines down to bypass syntax filters
                    vals = []
...
try:
    ...
    if isinstance(selected_range, tuple) and len(selected_range) == 2:
        ...
        if is_admin:
            ...
            # --- ⚠️ ACCOUNT PROFITABILITY ALERTS BLOCK ---
    ...
    st.error(f"Google Sheet Connection Error: {e}. Ensure you have created the 'Clockify_Data' tab in your sheet.")                m_clock_cp['Is_Internal'] = m_clock_cp['Client'].str.strip().str.lower() == 'internal'
